...
var mergeSort = function (array) {
  // Your code here.
  //split array in half
  //if array length is 2 then sort
  //if array[0] larger than array[1], swap
  //
  var result = [];
  if (array.length <= 1) {
    return array;
  ...
  var first = array.splice(0, half)
  var second = array.splice(-half)
  ...
  if (second.length) {
    second = result.concat(second);
  }
  // return merge(merge(first), merge(second));
  // if (first > second) {
  //   var before = first;
  //   first = second;
  //   second = before
