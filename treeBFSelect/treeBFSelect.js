...
Tree.prototype.BFSelect = function(filter) {
  ...
  var result =[];
  // var queue = [];
  // console.log({value:this, depth:0})
  queue.push({value:this, depth:0});

  var recurseTree = function (queue) {
    if (queue.length <= 0) {
      return;
    }
    var node = queue.pop();
    ...
    for (var i = 0; i < tree.children.length; i++) {
      queue.push({value:tree.children[i], depth: depth +1})
    }
  }
 recurseTree(queue)
  return result;
